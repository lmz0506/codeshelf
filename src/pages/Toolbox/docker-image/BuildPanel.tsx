...
export function BuildPanel({ model }: BuildPanelProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
      <div className="grid items-center gap-3 lg:grid-cols-[48px_minmax(0,1fr)_180px_132px_112px]">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600">
          <Package size={18} />
        </div>
        <div className="min-w-0">
          <div className="mb-1 flex items-center gap-2">
            <span className="text-sm font-semibold text-gray-900">构建镜像</span>
            <span className="truncate font-mono text-xs text-gray-400">{state.fullImageName || "image:tag"}</span>
          </div>
          <Input
        ...
        <label className="mt-5 flex h-8 items-center gap-2 rounded-lg border border-gray-200 px-3 text-xs text-gray-600 lg:mt-6">
          <input
            type="checkbox"
        ...
        <Button onClick={actions.buildImage} disabled={state.busy || !state.status?.available} variant="primary" size="sm" className="mt-5 h-8 lg:mt-6">
