...
pub(super) fn groups_file() -> Result<PathBuf, String> {
    Ok(get_storage_config()?.api_groups_file())
}

pub(super) fn endpoints_file() -> Result<PathBuf, String> {
    Ok(get_storage_config()?.api_endpoints_file())
}

pub(super) fn sessions_dir() -> Result<PathBuf, String> {
...
pub(super) fn load_groups() -> Result<Vec<ApiGroup>, String> {
...
pub(super) fn write_groups(groups: &[ApiGroup]) -> Result<(), String> {
    let path = groups_file()?;
    ensure_parent(&path)?;
    let content =
...
pub(super) fn load_endpoints() -> Result<Vec<ApiEndpoint>, String> {
...
pub(super) fn write_endpoints(endpoints: &[ApiEndpoint]) -> Result<(), String> {
    let path = endpoints_file()?;
    ensure_parent(&path)?;
