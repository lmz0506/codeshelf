...
async fn fetch_project_by_id(id: &str) -> Result<Option<Project>, String> {
    let pool = pool();
    let row: Option<ProjectRow> = sqlx::query_as(&format!("{} WHERE id = ?", PROJECT_SELECT))
        .bind(id)
        .fetch_optional(pool)
        .await
        .map_err(|e| format!("查询项目 {} 失败: {}", id, e))?;
    ...
    let labels: Vec<String> =
        sqlx::query_scalar("SELECT label FROM project_labels WHERE project_id = ? ORDER BY label")
            ...
            .fetch_all(pool)
            .await
            .map_err(|e| format!("查询 labels 失败: {}", e))?;

    Ok(Some(project_from_row(row, tags, labels)))
}

/// 取所有项目（一次查询拉全部 + 各拉一次 tags/labels，避免 N+1）
async fn fetch_all_projects() -> Result<Vec<Project>, String> {
    let pool = pool();
    ...
    let all_tags: Vec<(String, String)> =
        sqlx::query_as("SELECT project_id, tag FROM project_tags ORDER BY project_id, tag")
            .fetch_all(pool)
            .await
            .map_err(|e| format!("查询全量 tags 失败: {}", e))?;
    ...
    .fetch_all(pool)
    .await
    .map_err(|e| format!("查询全量 labels 失败: {}", e))?;
...
async fn project_exists(id: &str) -> Result<bool, String> {
    let exists: Option<(i64,)> = sqlx::query_as("SELECT 1 FROM projects WHERE id = ?")
        .bind(id)
        .fetch_optional(pool())
        .await
        .map_err(|e| format!("查询项目存在性失败: {}", e))?;
    Ok(exists.is_some())
}
...
pub async fn create_project(input: CreateProjectInput) -> Result<Project, String> {
    ...
    for label in &labels {
        ...
        .execute(&mut *tx)
        .await
        .map_err(|e| format!("插入 label 失败: {}", e))?;
    }
...
pub async fn update_project(input: UpdateProjectInput) -> Result<Project, String> {
    ...
    if let Some(name) = &input.name {
        ...
        sqlx::query("UPDATE projects SET updated_at = ? WHERE id = ?")
            ...
            .execute(&mut *tx)
            .await
            .map_err(|e| format!("更新 updated_at 失败: {}", e))?;
    }
    ...
    if let Some(tags) = &input.tags {
        sqlx::query("DELETE FROM project_tags WHERE project_id = ?")
            ...
            .execute(&mut *tx)
            .await
            .map_err(|e| format!("清空旧 tags 失败: {}", e))?;
        for tag in tags {
            sqlx::query(
    ...
    if let Some(labels) = &input.labels {
        sqlx::query("DELETE FROM project_labels WHERE project_id = ?")
            ...
            .execute(&mut *tx)
            .await
            .map_err(|e| format!("清空旧 labels 失败: {}", e))?;
        for label in labels {
            sqlx::query(
                "INSERT INTO project_labels (project_id, label) VALUES (?, ?) ON CONFLICT DO NOTHING",
            )
...
pub async fn update_last_opened(id: String) -> Result<Project, String> {
    ...
    .execute(pool())
    .await
    .map_err(|e| format!("更新 last_opened 失败: {}", e))?;
    ...
    for input in updates {
        ...
        if let Some(labels) = &input.labels {
            sqlx::query("DELETE FROM project_labels WHERE project_id = ?")
                ...
                .execute(&mut *tx)
                .await
                .map_err(|e| format!("清空 labels 失败: {}", e))?;
            for label in labels {
                sqlx::query(
                    "INSERT INTO project_labels (project_id, label) VALUES (?, ?) ON CONFLICT DO NOTHING",
                )
...
pub async fn set_project_editor(
    ...
    .execute(pool())
    .await
    .map_err(|e| format!("更新 editor_id 失败: {}", e))?;
...
pub async fn set_project_claude_env(
    ...
    .execute(pool())
    .await
    .map_err(|e| format!("更新 claude_env_name 失败: {}", e))?;
