...
pub fn apply_pending_restore(data_dir: &Path) -> Result<(), String> {
    ...
    let timestamp = fs::read_to_string(&flag)
        .map_err(|e| format!("读取 restore 标记失败: {}", e))?
        .trim()
        .to_string();
    if timestamp.is_empty() {
        let _ = fs::remove_file(&flag);
        return Err("restore 标记内容为空".to_string());
    }
    ...
    if !backup_dir.exists() {
        let _ = fs::remove_file(&flag);
        return Err(format!("备份 {} 已不存在", timestamp));
    }
...
/// 列举所有可用备份的时间戳（按新到旧排序）
pub fn list_backup_timestamps(data_dir: &Path) -> Result<Vec<String>, String> {
    let parent = data_dir
        .parent()
        .ok_or_else(|| "无法定位 data_dir 父目录".to_string())?;
    if !parent.exists() {
    ...
    let mut out = Vec::new();
    for entry in fs::read_dir(parent).map_err(|e| format!("读取备份目录失败: {}", e))? {
        let entry = entry.map_err(|e| format!("读取条目失败: {}", e))?;
...
fn clear_dir_keeping(dir: &Path, keep_names: &[&str]) -> Result<(), String> {
    if !dir.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(dir).map_err(|e| format!("读取 data_dir 失败: {}", e))? {
        let entry = entry.map_err(|e| format!("读取条目失败: {}", e))?;
        let name = entry.file_name().to_string_lossy().to_string();
        ...
        let ft = entry
            .file_type()
            .map_err(|e| format!("读取类型失败 {:?}: {}", path, e))?;
        if ft.is_dir() {
